...
/// <summary>
/// Everything the worker registers, in the order it registers it.
/// </summary>
/// <remarks>
/// <para>
/// One method rather than the body of <c>Program</c>, so that a test can build exactly the
/// container the worker runs with and ask it what it resolved. The worker once ran with an
/// empty connector registry because two extension methods both used <c>TryAdd</c> and the
/// one that mattered came second: every component had tests, every test built its own
/// container, and the only container nobody tested was this one. A demand sat queued on a
/// live stack until somebody read the log.
/// </para>
/// <para>
/// Nothing here is different from what <c>Program</c> did; it has moved, not changed.
/// </para>
/// </remarks>
