...
## The workers have no way to ask for anything

**Required by:** §6.7 defines three worker-facing routes behind mutual TLS — a job-scoped vault
release, and callbacks reporting that a job succeeded or failed. §2 draws the scan worker and the
removal worker as their own processes, and §5's sequence has a worker asking the vault service for
a scoped release at the moment it needs one, rather than holding standing access.

**Today:** none of those routes exist and nothing builds them. The workers deliberately hold no
key-manager credentials and no vault connection string — `docker-compose.yml` says so beside the
worker's environment, and the OpenBao policy grants decrypt to the API alone. That is the right
posture: a process driving browsers against third-party sites is the last one that should be able
to decrypt an identity. It also means a worker that needs somebody's name has to ask the process
holding the keys, and **there is nothing to ask.** `IBrokerSearch` now states exactly what a search
is handed; there is no path that hands it over.

**The authorization is the harder half, and it is worth separating from the transport.** The
obvious reading of §6.7 is that mutual TLS authenticates the worker and the release then names a
job, which the API resolves to an account. Resolving it requires reading a row before the caller is
acting for any tenant, which this schema does exactly twice — `app.find_passkey` and
`app.find_refresh_token` — and both are safe for a reason that does not carry over: their argument
is an unguessable secret, so presenting it *is* the authentication and the function only turns it
into a name. A scan id is not a secret. It is generated by the service and handed straight back to
the client by `POST /scans`. A definer function keyed on one would be the weakest of the three, and
everything holding it up would be the certificate.

**What closing it involves, roughly:** a second listener the public edge never routes to, client
certificates as its authentication, and a typed client on the worker side. The structural half is
that an internal route should be *absent* from the public listener rather than refused by it —
a route that exists and says 403 is one deployment mistake away from saying 200. The grant itself
is better modelled as a capability than a lookup: minted when a leg of work is dispatched, stored
as a digest, redeemed once and within a window, which is the shape refresh-token rotation already
has here and which makes the release's safety independent of what the transport proves.

**Why it went unnoticed:** every phase that needs it was itself deferred or unbuilt. The removal
worker waits on the job model, the scoped release was deferred for the same reason, and nothing
scanned at all — so the absence never had anything standing on it.
